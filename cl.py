...
def getresults(query):
	...
	if results_found != "no results":
		...
		print "Found {} results for \"{}\"".format(total_results, 
											query.replace("_", " "))
		...
		if len(total_results) >= 1:			
			if len(total_results) == 4:
				...
				more_results = False
				listing_title = [link.text for link in makesoup(url).find_all(
...
def averageprice(price, lowhigh=None):
	...
	if type(price) == list:		
		...
		if lowhigh:
			sorted_items = sorted(price.items(), 
								  key = lambda (key,value): int(value))
			print "Low price: \n{} \nHigh price: \n{}".format(sorted_items[0], 
															  sorted_items[-1])
	else:
	...
	if type(dic) == dict:
		...
		for key, value in dic.items():
			if type(query) ==  str or int:
				...
				raise TypeError(
					"Got {} expected str or int for query".format(type(dic)))
		return filtered_dic
	else:
		raise TypeError("Got {} expected dict".format(type(dic)))

def sortdata(dic, descending=None):
	"""Sort results in ascending or descendng order"""
	if type(dic) == dict:
		if descending:
			return sorted(price.items(), 
						  key = lambda (key,value): int(value), 
						  reverse = True)
		else:
